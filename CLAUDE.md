...
- **Sync**: Flux reacts to pushes to `master` immediately via a webhook receiver — no need to wait out the `interval` or manually run `flux reconcile` after pushing; reconciliation typically starts within seconds.
- **CI**: PRs run `flate` diff/test (`.github/workflows/flate.yaml`); Renovate auto-bumps images per `.renovate/` rules; non-trivial Renovate version bumps are gated by a Claude review (`.github/workflows/renovate-review.yaml`, `claude/renovate-review` status — digest/github-action/grafana-dashboard updates are ungated); use `/tune-renovate-review` to analyze recent CI runs for prompt/tier improvements and `/tune-advisory-gate` to check the advisory-finding merge gate's precision against real review bodies
