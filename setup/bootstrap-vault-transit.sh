...
initVault() {
  ...
  if [ "$init_status" == "false" ]; then
    ...
    vault_init=$(kubectl -n kube-system exec "vault-transit-0" -- vault operator init -format json -recovery-shares=1 -recovery-threshold=1) || exit 1
    export VAULT_TRANSIT_RECOVERY_TOKEN=$(echo $vault_init | jq -r '.recovery_keys_b64[0]')
  ...
  if [ "$sealed_status" == "true" ]; then
    ...
    kubectl -n kube-system exec "vault-transit-0" -- vault operator unseal "$VAULT_TRANSIT_RECOVERY_TOKEN" || exit 1
  fi
...
setupVaultTransitServer() {
  ...
  export VAULT_UNSEAL_TOKEN=$(VAULT_TOKEN=$"WRAPPING_TOKEN" vault unwrap -wrap-ttl=43800h) || exit 1
