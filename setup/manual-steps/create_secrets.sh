...
kubectl create secret generic traefik-basic-auth-jeff --from-literal=auth="$JEFF_AUTH" --namespace logs --dry-run -o json | kubeseal --format=yaml --cert=../pub-cert.pem > ../../secrets/basic-auth-jeff-logs.yaml
...
kubectl create secret generic cloudflare-api-key --from-literal=api-key="$CF_API_KEY" --namespace cert-manager --dry-run -o json | kubeseal --format=yaml --cert=../pub-cert.pem > ../../secrets/cloudflare-api-key.yaml
...
kseal values-to-encrypt/prometheus-operator-values.txt > ../../secrets/prometheus-operator-values.yaml
